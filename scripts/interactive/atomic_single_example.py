...
if __name__ == "__main__":
    ...
    #ADRIAN ADDED
    data_loader, text_encoder = interactive.load_data("atomic", opt, args.loader_path)
    ###
    ...
    with open("COMETTags.json", 'w') as f:
        f.write("[\n")

    #READ INPUT FILE
    with open("InputEvents.txt", 'r',-1, "utf8") as rf:
        EventN=-1
        first=True
        for input_event in rf:
            EventN+=1
            if first:
                first=False;
            else:
            ...
            input_event=input_event.rstrip('\n')#remove last character from input event cuz its newline char
            print(str(EventN) + ".  " + input_event)
    #############################
    ...
    #while True:
        ...
        #####################################
        ########ADRIAN ADDED
            sampling_algorithm="beam-5"#other options: greedy, beam-#, topk-#  (#=number)
        #############
            sampler = interactive.set_sampler(opt, sampling_algorithm, data_loader)

        #######if category not in data_loader.categories:
            ######category = "all"
        ########ADRIAN ADDED -- saving results as json only works if category is all
            category = "all"
        #########
