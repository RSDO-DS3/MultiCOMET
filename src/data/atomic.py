...
class GenerationDataLoader(DataLoader):
    ...
    def load_data(self, path):
        if ".pickle" in path:#load existing data loader
    ...
    def make_tensors(self, text_encoder, special,
        ...
        ###ADRIAN CODE:
        ####IMPORANT NOTES:
        ###MAX EVENT BECOMES MAXE1 IN NAME OF LOADER, REMEMBER THIS WHEN ADJUSTING THE LOADER BEING USED IN TRAINING PROGRAM
        ###MAX EFFECT BECOMES MAXE2 IN NAME OF LOADER, REMEMBER THIS WHEN ADJUSTING THE LOADER BEING USED IN TRAINING PROGRAM
        ###DONT CHANGE MAX EFFECT --GET ERRORZ

        print("MAX EVENT IN DATA:" + str(max([max([l[0] for l in self.masks[split]["total"]])
                              for split in self.masks])))

        self.max_event=50
        print("MAX EVENT SET: "+ str(self.max_event))
        ################################################################################
